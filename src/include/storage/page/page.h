...
class Page {
 ...
 public:
  ...
  /** Set the is_dirty. */
  inline void SetIsDirty(bool is_dirty){ 
    WLatch();
    is_dirty_ = is_dirty;
    WUnlatch();
  }

  /** Decrese the pin_count. */
  inline void DecPinCnt(){ 
    WLatch();
    pin_count_--;
    WUnlatch();
  }

  /** Increase the pin_count. */
  inline void IncPinCnt(){ 
    WLatch();
    pin_count_++;
    WUnlatch();
  }
  ...
  /** Reset the page . */
  inline void Reset(page_id_t page_id) { 
    WLatch();
    ResetMemory();
    pin_count_ = 0;
    is_dirty_ = false;
