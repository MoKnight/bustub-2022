...
class TrieNode {
 public:
  ...
  bool HasChild(char key_char) const { 
    return children_.find(key_char) != children_.end()?true:false; 
  }
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) { 
    std::pair<std::unordered_map<char, std::unique_ptr<TrieNode>>::iterator,bool>  iterator;
    iterator = children_.emplace(key_char,child);
    if( iterator.second )
    {
...
class TrieNodeWithValue : public TrieNode {
 ...
 public:
   ...
   */
  TrieNodeWithValue(char key_char, T value) :key_char_(key_char),is_end_(true),value_(value){
  }
...
class Trie {
 ...
 public:
   ...
   * character.
   */
  Trie():root_(std::unique_ptr<TrieNode,std::default_delete<bustub::TrieNode>>(new TrieNode('/0'))){
  }
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for( int i = 0;i < length;i++){
      if( i == length - 1 ){
        if( cur_node->get()->HasChild(key.at(i))) {
          std::unique_ptr<TrieNode>* cur_node_child = cur_node->get()->GetChildNode(key.at(i));
          if( cur_node->get()->IsEndNode() ){
            return false; //Duplicated keys
          }
          /*
            before you insert,you should reomve first
          */
          TrieNodeWithValue<T> tmp_node(std::move(*cur_node_child->get()), value);
          cur_node->get()->RemoveChildNode(key.at(i));
          cur_node->get()->InsertChildNode(key.at(i),std::unique_ptr<TrieNode>(new TrieNode(std::move(tmp_node))));
          return true;
        } else {
          TrieNodeWithValue<T> tmp_node(key.at(i), value);
          cur_node->get()->InsertChildNode(value,tmp_node);
          return true;
        }
      } else {
        if(cur_node->get()->HasChild(key.at(i))){
          cur_node = cur_node->get()->GetChildNode(key.at(i)); 
        } else {
          TrieNode tmp_node(key.at(i));
          cur_node->get()->InsertChildNode(key.at(i),std::unique_ptr<TrieNode>(new TrieNode(std::move(tmp_node))));
          cur_node = cur_node->get()->GetChildNode(key.at(i)); 
        }
      }
  ...
  bool Remove(const std::string &key) { 
    latch_.WLock();
    std::unique_ptr<TrieNode>* res_node = Get(key);
    if(res_node == nullptr){
      latch_.WUnlock();
      return false;
    }
    
    res_node->get()->RemoveChildNode(key.back());
    res_node->get()->SetEndNode(false);
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    TrieNodeWithValue<T>* casted_res_node = dynamic_cast<TrieNodeWithValue<T>*>(res_node->get());
    if( rcasted_res_node != nullptr )
    {
      latch_.RUnlock();
      *success = true;
      return casted_res_node->GetValue();
    }
   ...
   * @brief 
   * 
   * attention,you must have the lock before you go into this func
   * 
   * @param key 
   * @return std::unique_ptr<TrieNode>* 
   */
  std::unique_ptr<TrieNode>* Get(const std::string &key){
    if(key.empty())
    {
      return nullptr;
    } 
   ...
   * @return false 
   */
  bool RecursiveRemove(std::unique_ptr<TrieNode>* cur_node, const std::string &key,int i){
    bool tmp_res = true;
    if( i == key.size() - 2 ){
        if( cur_node->get()->HasChild(key.at(i+1)) ){
          std::unique_ptr<TrieNode>* cur_node_child = cur_node->get()->GetChildNode(key.at(i));
          if( cur_node_child->get()->IsEndNode() ){
            cur_node_child->get()->SetEndNode(false);
            return true;
          } else {
            return false;
          }
    ...
    if( cur_node->get()->HasChild(key.at(i)) ) {
        std::unique_ptr<TrieNode>* cur_node_child = cur_node->get()->GetChildNode(key.at(i));
        bool tmp_res = RecursiveRemove(cur_node_child, key, i++);
        if( cur_node_child->get()->HasChildren() && !cur_node_child->get()->IsEndNode()){
          cur_node->get()->RemoveChildNode(key.at(i));
        }
    }
